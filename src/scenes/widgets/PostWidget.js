...
const PostWidget = ({
  ...
  const patchLike = async () => {
    const response = await fetch(`https://linkup-api-pojo.onrender.com/post/${postId}/like`, {
      method: "PATCH",
      headers: {
  ...
  const getAndSetComments = async () => {
    const res = await fetch(
      `https://linkup-api-pojo.onrender.com/post/comment/getCommentBypost/${postId}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    <WidgetWrapper m="2rem 0">
      ...
      {picturePath && (
        <img
          ...
          src={`https://linkup-api-pojo.onrender.com/assets/${picturePath}`}
        />
      )}
